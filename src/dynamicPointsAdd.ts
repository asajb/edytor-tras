window.addEventListener("DOMContentLoaded", () => {
    ...
    const totalFormsInput = document.getElementById("id_form-TOTAL_FORMS") as HTMLInputElement;
    ...
    if(mapImage) {
        mapImage.addEventListener("click", (event: MouseEvent) => {
            ...
            if (emptyFormDiv && formsetTableBody) {
                ...
                const xElements = tempDiv.querySelector('[name$="-x"]')?.parentElement;
                const yElements = tempDiv.querySelector('[name$="-y"]')?.parentElement;
                
                if (xElements) cell1.appendChild(xElements);
                if (yElements) cell2.appendChild(yElements);
                
                newRow.appendChild(cell1);
                newRow.appendChild(cell2);
                
                // Set the coordinate values
                const xInput = newRow.querySelector('input[name$="-x"]') as HTMLInputElement;
                const yInput = newRow.querySelector('input[name$="-y"]') as HTMLInputElement;
